...
> Provenance: extracted from `C:/Users/philm/.local/bin/claude.exe` (the
> installed 2026-07-25 build) by the design pass on 2026-08-01. Written down
> because it was expensive to obtain and will silently rot otherwise.
...
`Alt+E` pressed while the files view is already showing reloads the open file,
so it is never a key that does nothing.
